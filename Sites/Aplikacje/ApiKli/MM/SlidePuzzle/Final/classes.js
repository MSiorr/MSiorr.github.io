class PuzzleStructure {
        ...
        for (let i = 0; i < this.count; i++) {
            ...
            for (let j = 0; j < this.count; j++) {
                if (i != this.count - 1 || j != this.count - 1) {
                    ...
                    div.style.backgroundImage = "url('./img/image" + imgIndex + ".png')";
    ...
    SamuraiRewardPreload(){
        mp4Samurai.id = "mp4samurai";
        mp4Samurai.loop = "true";
        mp4Samurai.type = "video/mp4";
        mp4Samurai.preload = "auto";
        mp4Samurai.src = './img/reward/movie.mp4';
        mp4Samurai.autoplay = "true";
        arena.appendChild(mp4Samurai);
    }
}
...
class CustomAlerts {
    ...
    static winAlert(scoreTime) {
        ...
        inputName.placeholder = "Podaj nick"
    ...
    static ShowRanking() {
        ...
        let titles = ["Witcher 3", "Cyberpunk 2077", "Sekiro", "World Of Tanks"]

        let imgRanking = document.createElement("img")
        imgRanking.id = "imgRanking";
        imgRanking.src = "./img/image" + imgIndex + ".png";
        ...
        for (let i = 0; i < btnList.length; i++) {
            ...
            button.addEventListener("click", () => {
                ...
                for (let i = 0; i < 10; i++) {
                    if (i == 0) {
                        rankingTable.innerHTML = "<tr><th>Pos.</th><th>Nick</th><th>Moves</th><th>Time</th></tr>"
                    }
                    let nick = "--------------------";
                    let moves = "_____"
                    ...
                    if (currentImgCookie != null) {
                        if (currentImgCookie.value[mode] != undefined) {
                            if (i < currentImgCookie.value[mode].length) {
                                nick = currentImgCookie.value[mode][i][0];
...
class Cookies {
    ...
    CheckAddTimeToCookie(nick, time, moves, size) {
        ...
        for (let i = 0; i < this.imagesCookieList.length; i++) {
            if (currentImgIndex == this.imagesCookieList[i].name) {
                ...
                if (this.imagesCookieList[i].value[stringMode] != undefined) {
                    ...
                    if (canAdd == true) {
                        ...
                        for (let j = this.imagesCookieList[i].value[stringMode].length - 1; j >= index; j--) {
                            if (j != index) {
                                ...
                                this.imagesCookieList[i].value[stringMode][j][0] = nick
                    ...
                    this.imagesCookieList[i].value[stringMode] = [[nick, time, moves]]
                }
            }
        }
        if (added == false) {
            let val = JSON.stringify({ [stringMode]: [[nick, time, moves]] })
